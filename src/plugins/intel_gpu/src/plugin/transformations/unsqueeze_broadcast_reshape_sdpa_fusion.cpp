...
UnsqueezeBroadcastReshapeSDPAFusion::UnsqueezeBroadcastReshapeSDPAFusion() {
    ...
    auto input_c_any_m = any_input();

    auto axes_const_b_m = wrap_type<ov::op::v0::Constant>();
    auto axes_const_b2_m = wrap_type<ov::op::v0::Constant>();
    auto axes_const_c_m = wrap_type<ov::op::v0::Constant>();
    auto axes_const_c2_m = wrap_type<ov::op::v0::Constant>();
    auto unsqueeze_b_m = wrap_type<ov::op::v0::Unsqueeze>({input_b_kvcache_m, axes_const_b_m}, unsqueeze_predicate);
    auto unsqueeze_b_any_m = wrap_type<ov::op::v0::Unsqueeze>({input_b_any_m, axes_const_b2_m}, unsqueeze_predicate);
    auto unsqueeze_c_m = wrap_type<ov::op::v0::Unsqueeze>({input_c_kvcache_m, axes_const_c_m}, unsqueeze_predicate);
    auto unsqueeze_c_any_m = wrap_type<ov::op::v0::Unsqueeze>({input_c_any_m, axes_const_c2_m}, unsqueeze_predicate);
    ...
    auto broadcast_input_b_m = std::make_shared<ov::pass::pattern::op::Or>(OutputVector{unsqueeze_b_m, unsqueeze_b_any_m, pre_reshape_b_m});
    auto broadcast_input_c_m = std::make_shared<ov::pass::pattern::op::Or>(OutputVector{unsqueeze_c_m, unsqueeze_c_any_m, pre_reshape_c_m});
    ...
    ov::matcher_pass_callback callback = [OV_CAPTURE_CPY_AND_THIS](ov::pass::pattern::Matcher& m) {
        ...
        else if (pattern_map.find(input_b_rope_m) != pattern_map.end())
            data_inputs.push_back(pattern_map.at(input_b_rope_m).get_node_shared_ptr());      // K input from RoPE
        else if (pattern_map.find(input_b_any_m) != pattern_map.end())
            data_inputs.push_back(pattern_map.at(input_b_any_m).get_node_shared_ptr());       // K input from Concat or other
        ...
        else if (pattern_map.find(input_c_transpose_m) != pattern_map.end())
            data_inputs.push_back(pattern_map.at(input_c_transpose_m).get_node_shared_ptr()); // V input from Transpose
        else if (pattern_map.find(input_c_any_m) != pattern_map.end())
            data_inputs.push_back(pattern_map.at(input_c_any_m).get_node_shared_ptr());       // V input from Concat or other
