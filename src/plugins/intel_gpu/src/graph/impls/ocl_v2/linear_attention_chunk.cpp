...
//
#include "linear_attention_chunk.hpp"
...
constexpr size_t default_chunk_size = 64;
...
size_t get_chunk_size() {
    static const size_t chunk_size = [] {
        const char* env = std::getenv("OV_GENAI_CHUNK_LINEAR_ATTN_SIZE");
        if (env) {
            try {
...
class LinearAttentionChunkGenerator : public KernelGenerator {
public:
    LinearAttentionChunkGenerator() : KernelGenerator("linear_attention_chunk") {}
...
protected:
    [[nodiscard]] JitConstants get_jit_constants(const RuntimeParams& params) const override {
        ...
        jit.make("CHUNK_SIZE", get_chunk_size());
...
class LinearAttentionChunkImpl : public PrimitiveImplOCL {
public:
    DECLARE_OBJECT_TYPE_SERIALIZATION(ov::intel_gpu::ocl::LinearAttentionChunkImpl)

    Stage::Ptr linear_attn_chunk = make_stage<LinearAttentionChunkGenerator>();

    LinearAttentionChunkImpl() : PrimitiveImplOCL(LinearAttentionChunk::get_type_info_static()) {}
    LinearAttentionChunkImpl(const program_node& node, const RuntimeParams& params) : LinearAttentionChunkImpl() {
        add_stage(linear_attn_chunk, params);
    }
    ...
    [[nodiscard]] std::unique_ptr<primitive_impl> clone() const override {
        return make_deep_copy<LinearAttentionChunkImpl>(this);
    }
};

}  // namespace

std::unique_ptr<primitive_impl> LinearAttentionChunk::create_impl(const program_node& node, const RuntimeParams& params) const {
    assert(node.is_type<linear_attention>());
    return std::make_unique<LinearAttentionChunkImpl>(node, params);
}

}  // namespace ov::intel_gpu::ocl

BIND_BINARY_BUFFER_WITH_TYPE(ov::intel_gpu::ocl::LinearAttentionChunkImpl)
