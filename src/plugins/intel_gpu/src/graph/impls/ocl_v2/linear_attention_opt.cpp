...
public:
    LinearAttentionOptGenerator() : KernelGenerator("linear_attention_opt") {}
...
class LinearAttentionOptImpl : public PrimitiveImplOCL {
public:
    DECLARE_OBJECT_TYPE_SERIALIZATION(ov::intel_gpu::ocl::LinearAttentionOptImpl)

    Stage::Ptr linear_attn_opt = make_stage<LinearAttentionOptGenerator>();

    LinearAttentionOptImpl() : PrimitiveImplOCL(LinearAttentionOpt::get_type_info_static()) {}
    LinearAttentionOptImpl(const program_node& node, const RuntimeParams& params) : LinearAttentionOptImpl() {
        add_stage(linear_attn_opt, params);
    }
    ...
    [[nodiscard]] std::unique_ptr<primitive_impl> clone() const override {
        return make_deep_copy<LinearAttentionOptImpl>(this);
    }
};

}  // namespace

std::unique_ptr<primitive_impl> LinearAttentionOpt::create_impl(const program_node& node, const RuntimeParams& params) const {
    assert(node.is_type<linear_attention>());
    return std::make_unique<LinearAttentionOptImpl>(node, params);
}

}  // namespace ov::intel_gpu::ocl

BIND_BINARY_BUFFER_WITH_TYPE(ov::intel_gpu::ocl::LinearAttentionOptImpl)
